...
mongoose_1.default.connect(MONGODB_URI)
    ...
    .catch((err) => console.error('Bot failed database connection:', err));
 ...
 */
async function ensureUserExists(ctx, referrerId) {
    const tgUser = ctx.from;
    if (!tgUser)
        throw new Error('No user context found');
    let user = await user_model_1.User.findOne({ telegramId: tgUser.id.toString() });
    if (!user) {
        ...
        if (validReferrerId) {
            const referrer = await user_model_1.User.findOne({ telegramId: validReferrerId });
            if (referrer) {
                ...
                try {
                    await bot.telegram.sendMessage(validReferrerId, `🎉 *New Invite!* @${tgUser.username || tgUser.first_name} joined using your link.\n\nThey need to complete their first earning task to release your *₹40* and their *₹20* bonus!`, { parse_mode: 'Markdown' });
                }
                catch (e) {
...
bot.start(async (ctx) => {
    try {
        ...
        const inlineKeyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.webApp('🚀 Open PaisaTap App', webAppUrlForTelegram)],
            [
                telegraf_1.Markup.button.callback('💰 Check Balance', 'check_balance'),
                telegraf_1.Markup.button.callback('👥 Refer Friends', 'refer_friends')
            ]
        ]);
...
bot.command('balance', async (ctx) => {
    try {
        ...
        const msg = `💸 *PaisaTap Balance Log*\n\n` +
...
bot.command('tasks', async (ctx) => {
    try {
        ...
        const activeTasks = await task_model_1.Task.find({ isActive: true }).limit(3);
        ...
        const inlineKeyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.webApp('📋 Open Tasks Section', `${webAppUrlForTelegram}/#/tasks`)]
        ]);
        await ctx.replyWithMarkdown(msg, inlineKeyboard);
    }
    catch (error) {
...
bot.command('refer', async (ctx) => {
    try {
        ...
        const msg = `👥 *Invite Friends & Earn Cash!* 🎁\n\n` +
        ...
        const inlineKeyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.switchToChat('✉️ Share Link', refLink)],
            [telegraf_1.Markup.button.webApp('👥 Leaderboard', `${webAppUrlForTelegram}/#/refer`)]
        ]);
        await ctx.replyWithMarkdown(msg, inlineKeyboard);
    }
    catch (error) {
...
bot.command('leaderboard', async (ctx) => {
    try {
        const topEarners = await user_model_1.User.find({ totalEarned: { $gt: 0 } })
            .sort({ totalEarned: -1 })
            .limit(10);
        ...
        await ctx.replyWithMarkdown(msg, telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.webApp('🎮 Open App & Play', webAppUrlForTelegram)]
        ]));
    }
    catch (error) {
        console.error(error);
...
bot.command('help', async (ctx) => {
    const msg = `❔ *PaisaTap Support Guide*\n\n` +
