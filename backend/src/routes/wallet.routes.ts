...
router.post('/webhook', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const withdrawal = referenceId 
      ? await Withdrawal.findById(referenceId)
      : await Withdrawal.findOne({ razorpayPayoutId: payoutId });
...
router.get('/balance', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const withdrawals = await Withdrawal.find({ telegramId: tgUser.telegramId })
      .sort({ createdAt: -1 });
...
router.post('/withdraw', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const fee = paymentService.calculateFee(amount);
    ...
    try {
      ...
      withdrawal.status = payoutResult.status as any; // e.g. processing
      await withdrawal.save();
      ...
      if (payoutResult.mode === 'SIMULATOR') {
        setTimeout(async () => {
          try {
            const freshWithdrawal = await Withdrawal.findById(withdrawal._id);
